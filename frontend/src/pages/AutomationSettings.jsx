            ...
            <CardContent className="space-y-8">
              ...
              <div className="space-y-4">
                <div className="space-y-0.5">
                  ...
                  <p className="text-sm text-muted-foreground">How often to check for playlist updates and discover new streams</p>
                </div>
                <Tabs
                  value={typeof config?.playlist_update_interval_minutes === 'object' ? config.playlist_update_interval_minutes.type : 'interval'}
                  onValueChange={(val) => {
                    const currentVal = typeof config?.playlist_update_interval_minutes === 'object'
                      ? config.playlist_update_interval_minutes.value
                      : (config?.playlist_update_interval_minutes || 5);
                    const newVal = val === 'interval' ? (parseInt(currentVal) || 5) : (typeof currentVal === 'string' ? currentVal : '*/5 * * * *');
                    handleGlobalAutomationChange('playlist_update_interval_minutes', { type: val, value: newVal });
                  }}
                >
                  ...
                  <TabsContent value="interval" className="space-y-2 pt-4">
                    <div className="flex items-center gap-2">
                      <Input
                        ...
                        max="1440"
                        className="max-w-[120px]"
                        value={typeof config?.playlist_update_interval_minutes === 'object'
                          ? (config.playlist_update_interval_minutes.type === 'interval' ? config.playlist_update_interval_minutes.value : 5)
                          : (config?.playlist_update_interval_minutes || 5)}
                        onChange={(e) => {
                          const val = parseInt(e.target.value) || 1;
                          handleGlobalAutomationChange('playlist_update_interval_minutes', { type: 'interval', value: val });
                        }}
                      />
                      <span className="text-sm text-muted-foreground">minutes</span>
                    </div>
                  ...
                  <TabsContent value="cron" className="space-y-2 pt-4">
                    <div className="space-y-2">
                      <Input
                        placeholder="*/5 * * * *"
                        value={typeof config?.playlist_update_interval_minutes === 'object' && config.playlist_update_interval_minutes.type === 'cron'
                          ? config.playlist_update_interval_minutes.value
                          : ''}
                        onChange={(e) => handleGlobalAutomationChange('playlist_update_interval_minutes', { type: 'cron', value: e.target.value })}
                      />
                      <p className="text-xs text-muted-foreground">Standard 5-field cron expression (e.g., */5 * * * *)</p>
                    </div>
                  </TabsContent>
                </Tabs>
              </div>
